...
const Table: FC<TableProps> = (
    ...
    const getRowElementByIndex = useCallback((index: number): HTMLElement | undefined => {
    ...
    }, [focusIndex, getRowElementByIndex, isKeyboardControl, isRowLinkInteractive]);
        ...
        <Foreground foregroundClassName={classes.tableForeground}>
            ...
            <div role="grid" aria-rowcount={items.length} aria-label={tableLabeledBy}>
                <div className={classes.table__header} role="rowgroup">
                    <div className={classes.table__headerRow} role="row" aria-rowindex={1}>
                        {columns.map((column) => {
                            return (
                                <div
                                    key={column.key}
                ...
                >
                    {items.map((item, index) => {
                        ...
                        const row = columns.map(({ key, width, valueGetter }) => {
                            ...
                            return (
                                <div key={rowValue} style={{ minWidth: width }} className={classes.table__rowItem} role="cell">
                                    {rowValue}
                                </div>
                            );
                        });
